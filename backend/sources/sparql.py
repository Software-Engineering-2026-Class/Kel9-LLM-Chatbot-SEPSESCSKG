...
from backend.patterns import CAPEC_RE, CVE_RE, CWE_RE
...
CAPEC_RES = "http://w3id.org/sepses/resource/capec/"
...
def _capec_iri(capec_id: str) -> str:
    return f"<{CAPEC_RES}{capec_id}>"
...
def _capec_full(capec_id: str) -> str:
    return f"""{PREFIXES}
SELECT DISTINCT ?name ?description ?likelihood ?mitigation WHERE {{
  BIND({_capec_iri(capec_id)} AS ?capec)
  OPTIONAL {{ ?capec capec:name ?name . }}
  OPTIONAL {{ ?capec dct:description ?description . }}
  OPTIONAL {{ ?capec capec:likelihoodOfAttack ?likelihood . }}
  OPTIONAL {{ ?capec capec:hasMitigation ?mitigation . }}
}} LIMIT 50"""


def _severity_fallback(min_score: float = 9.0, limit: int = 15) -> str:
    """CVE dengan CVSS3 baseScore >= ambang (default 9.0 = critical; 7.0 = high)."""
    return f"""{PREFIXES}
SELECT DISTINCT ?cveId ?score WHERE {{
  ?cve a cve:CVE ; cve:hasCVSS3BaseMetric ?m . ?m cvss:baseScore ?score .
  FILTER(?score >= {float(min_score)})
  BIND(STRAFTER(STR(?cve), "{CVE_RES}") AS ?cveId)
}} ORDER BY DESC(?score) LIMIT {int(limit)}"""


def _high_severity_fallback() -> str:
    # Dipertahankan untuk kompatibilitas; setara critical (>= 9.0).
    return _severity_fallback(9.0, 10)
 
 
def _regex_sparql(question: str) -> Optional[str]:
    """Pemetaan deterministik NL -> SPARQL untuk pola yang sering & jelas.
    Mengurangi ketergantungan pada jalur LLM/fallback untuk pertanyaan baku."""
    q = question.lower()

    # 1) CVE eksplisit -> cabang detail / cvss / capec.
    ...
    # 2) CWE eksplisit -> daftar CVE yang memakai CWE itu.
    cwe = CWE_RE.search(question)
    if cwe:
        return _cves_by_cwe(cwe.group().upper())

    # 3) CAPEC eksplisit -> detail attack pattern.
    capec = CAPEC_RE.search(question)
    if capec:
        return _capec_full(capec.group().upper())

    # 4) Pertanyaan severity umum tanpa entitas (sering muncul: "critical/high").
    #    Tangani deterministik agar tidak selalu jatuh ke fallback_keyword.
    if any(k in q for k in ("critical", "kritis", "parah")):
        return _severity_fallback(9.0, 15)
    if "high severity" in q or "severity tinggi" in q or ("high" in q and "severity" in q):
        return _severity_fallback(7.0, 15)
...
def _balanced(query: str, open_ch: str, close_ch: str) -> bool:
    """Cek keseimbangan kurung, mengabaikan yang ada di dalam string literal."""
    depth = 0
    in_str = None  # menyimpan karakter pembuka string aktif: " atau '
    prev = ""
    for ch in query:
        if in_str:
...
def _valid_sparql(query: str) -> bool:
    """Validasi struktural dangkal-tetapi-berguna sebelum query dikirim ke endpoint.

    Menolak: operasi tulis (UPDATE), query tanpa SELECT/ASK, kurung tak seimbang,
    SELECT tanpa WHERE, atau query yang tampak terpotong (tak ada '}' penutup).
    Bukan parser SPARQL penuh — hanya menyaring keluaran LLM yang jelas rusak.
    """
    ...
    if _FORBIDDEN.search(query):
        return False
    up = query.upper()
    if not ("SELECT" in up or "ASK" in up):
        return False
    # SELECT wajib punya WHERE (ASK boleh tanpa kata WHERE eksplisit).
    if "SELECT" in up and "WHERE" not in up and "{" not in query:
        return False
